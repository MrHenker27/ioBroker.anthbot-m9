...
function updateTrackHistory(
    ...
    // A replay can straddle two curpath packets: the first copy may already be
    // stored at the end of the track while the next packet repeats the exact
    // same 3+ point sequence before continuing. Per-packet cleanup cannot see
    // that boundary, so run the same conservative adjacent-sequence collapse
    // once more on the combined track and then restore global track indexes.
    const collapsedTrack = collapseAdjacentReplaySequences(target);
    if (collapsedTrack.length !== target.length) {
        target.splice(0, target.length, ...collapsedTrack);
