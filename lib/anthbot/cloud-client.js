...
/**
 * Format an HTTP response payload for diagnostics without collapsing objects to "[object Object]".
 * @param {unknown} payload
 * @param {number} [maxLength]
 * @returns {string}
 */
function formatDiagnosticPayload(payload, maxLength = 600) {
    let text;
    try {
    ...
    if (typeof text !== 'string' || !text) text = String(payload);
...
class AnthbotCloudApiClient {
    ...
    constructor({ http, host, bearerToken = null }) {
        ...
        /** @type {null|(() => Promise<string|void>)} */
        this.authRefreshHandler = null;
    }

    /**
     * Register a callback that refreshes the account bearer token in-place.
     * @param {null|(() => Promise<string|void>)} handler
     */
    ...
    async getDeviceIotCredentials(serialNumber) {
        ...
        for (let attempt = 0; attempt < retryDelaysMs.length; attempt++) {
            ...
            try {
                ...
                const payload = response.data;
                if (response.status !== 200) {
                    const diagnostic = formatDiagnosticPayload(payload);
                    const tokenExpired =
                        response.status === 401 &&
                        /token\s+has\s+expired|token\s+expired/i.test(diagnostic);

                    if (tokenExpired && !authRefreshAttempted && this.authRefreshHandler) {
                        authRefreshAttempted = true;
                        await this.authRefreshHandler();
                        this.requireToken();
                        attempt = -1;
                        continue;
                    }

                    const error = new AnthbotGenieError(`IoT STS failed (${response.status}): ${diagnostic}`);
