...
class AnthbotCloudApiClient {
    ...
    /**
     * Downloads and extracts the Anthbot M9 map manager archive.
     *
     * @param {string} serialNumber
     * @returns {Promise<object>}
     */
      async getDeviceMap(serialNumber) {
        ...
        if (payload.code !== 0) {
            throw new AnthbotGenieError(
		`Map manager presigned URL returned code=${JSON.stringify(payload.code)} payload=${JSON.stringify(payload).slice(0,500)}`,
            );
        }
        ...
        if (mapResponse.status !== 200) {
            throw new AnthbotGenieError(
                `Map manager download failed (${mapResponse.status})`,
            );
        }

        const tempDir = await fs.mkdtemp(
            path.join(os.tmpdir(), `anthbot-m9-${serialNumber}-`),
        );
        ...
        try {
            ...
            const areaSettingPath = path.join(tempDir, 'area_setting.json');
            const rawText = await fs.readFile(areaSettingPath, 'utf8');

           const mapFilePath = path.join(tempDir, 'iot_map.bin');
           const mapBuffer = await fs.readFile(mapFilePath);
           const mapData = decodeM9Map(mapBuffer);

            const areaDefinition = JSON.parse(rawText);

            if (
                !areaDefinition ||
                typeof areaDefinition !== 'object' ||
                Array.isArray(areaDefinition)
            ) {
                throw new AnthbotGenieError(
                    'M9 area_setting.json payload type is not an object',
                );
            }
