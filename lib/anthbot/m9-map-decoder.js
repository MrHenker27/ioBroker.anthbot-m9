 ...
 *
 * @param {Buffer} buffer Binary map data
 * @returns {{
 *   headerLength: number,
 *   contourCount: number,
 *   mapInfo: {
...
function decodeM9Map(buffer) {
    ...
    const resolution = buffer.readFloatLE(offset);
    offset += 4;

    const minX = buffer.readFloatLE(offset);
    offset += 4;

    const minY = buffer.readFloatLE(offset);
    offset += 4;

    const mapId = buffer.readBigInt64LE(offset).toString();
    offset += 8;

    if (headerLength > buffer.length) {
        throw new Error(
            `Invalid M9 map header length ${headerLength} for file size ${buffer.length}`,
        );
    }
    ...
    for (let contourIndex = 0; contourIndex < contourCount; contourIndex++) {
        if (offset + 4 > buffer.length) {
            throw new Error(
                `Unexpected end of M9 map before contour ${contourIndex}`,
            );
        }

        const pointCount = buffer.readInt16LE(offset);
        offset += 2;
        ...
        if (pointCount < 0) {
            throw new Error(
                `Invalid negative point count ${pointCount} in contour ${contourIndex}`,
            );
        }
        ...
        if (offset + requiredBytes > buffer.length) {
            throw new Error(
                `Unexpected end of M9 map in contour ${contourIndex}: ` +
