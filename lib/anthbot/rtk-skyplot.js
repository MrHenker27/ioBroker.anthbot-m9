...
/**
 * Return a deterministic satellite list. The cloud may deliver the same
 * satellites in a different order on each refresh, which would otherwise
 * rewrite the JSON, SVG and Admin HTML states without a semantic change.
 * @param {Array<object>} satellites
 * @returns {Array<object>}
 */
function normalizeRtkSatellites(satellites) {
    const list = Array.isArray(satellites) ? satellites : [];
    return [...list].sort((left, right) => {
        const fields = ['satelliteId', 'constellation', 'azimuth', 'elevation', 'signalStrength'];
...
function renderRtkSkyplot(satellites, options = {}) {
    const list = normalizeRtkSatellites(satellites);
    const cx = 180;
    const cy = 180;
    const radius = 145;
...
module.exports = { normalizeRtkSatellites, renderRtkSkyplot };
