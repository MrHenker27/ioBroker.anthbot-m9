...
const { resolveCurrentManualZone } = require('../anthbot/m9-area-geometry');
...
function buildDeviceStateUpdates({ context, data, eventCodeCache, errorDescriptionLanguage, now }) {
    ...
    const currentManualZone = resolveCurrentManualZone(context.areaDefinition || {}, data, pose, {
        source: position.source,
        charging: isCharging(data),
    });
    ...
    return {
        ...
        'dashboard.currentZone': currentManualZone?.name || '',
        'dashboard.currentZoneId': currentManualZone?.id ?? null,
        'dashboard.currentZoneSource': currentManualZone?.source || '',
