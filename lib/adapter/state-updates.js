...
const { decodeCurpath } = require('../anthbot/curpath');
...
function buildDeviceStateUpdates({ context, data, eventCodeCache, errorDescriptionLanguage, now }) {
    ...
    const pose =
        data?.pose && typeof data.pose === 'object'
            ? data.pose
            : data?.anti_loss_pose?.pose2d && typeof data.anti_loss_pose.pose2d === 'object'
              ? data.anti_loss_pose.pose2d
              : {};
    const curpath = decodeCurpath(
        typeof data?.curpath?.value === 'string' ? data.curpath.value : '',
    );

    const livePose = curpath.lastPoint
        ? {
            x: curpath.lastPoint.xMetres,
            y: curpath.lastPoint.yMetres,
        }
        : pose;
    const consumables = consumableLifetimes(data);
    const mapData = context.mapData || {};
    const mapSvg = buildM9Svg(mapData, livePose);
    return {
        ...
        'map.svg': mapSvg,
        'map.lastUpdate': mapSvg ? Date.now() : null,
    };
}
