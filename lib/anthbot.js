 ...
 *
 * @param {string|number|boolean|null|undefined} value - Input value to convert
 ...
 *
 * @param {string|number|null|undefined} value - Seconds since epoch or formatted timestamp string
 ...
 *
 * @param {string|number|boolean|null|undefined} value - Input value
...
/**
 * Normalize a value into a finite number when possible.
 *
 * @param {string|number|null|undefined} value - Input value to convert
 * @returns {number|null} - Numeric value or null when conversion fails
 */
...
/**
 * Return the first defined, non-null value from a list.
 *
 * @param {...unknown} values - Candidate values
 * @returns {unknown} - First non-nullish value or null
 */
...
/**
 * Normalize a raw mower status value into a readable string.
 *
 * @param {string|number|null|undefined} value - Raw status value
...
/**
 * Determine whether the given model string refers to an Anthbot M-series mower.
 *
 * @param {string|null|undefined} deviceModel - Device model string
 * @returns {boolean} - True for M5/M9 models
 */
...
/**
 * Read the known param_set values from a mower payload without inventing defaults.
 *
 * @param {object} data - Device payload data
 * @returns {Record<string, number>} - Known param_set values
 */
...
/**
 * Build a safe param_set payload for the target mower family.
 *
 * Legacy Genie-family payloads keep their existing default-filled shape.
 * M-series payloads only send reported current values plus the requested patch.
 *
 * @param {string|null|undefined} deviceModel - Device model string
 * @param {object} data - Device payload data
 * @param {Record<string, number>} patch - Values to update
 * @returns {Record<string, number>} - Param_set payload
 */
...
/**
 * Read the known near-charger mow parameters from a mower payload.
 *
 * Uses the current Anthbot near-charger payload shape:
 * nest_switch, nest_mow_count, nest_cutter_height, nest_pobctl_switch, nest_pobctl_level.
 *
 * @param {object} data - Device payload data
 * @returns {Record<string, number>} - Known near-charger parameters
 */
...
/**
 * Build a near-charger mow payload for the set_mow_params command.
 *
 * @param {object} data - Device payload data
 * @param {Record<string, number>} patch - Values to update
 * @returns {Record<string, number>} - set_mow_params payload
 */
...
/**
 * Extract the raw M-series mode value.
 *
 * @param {object} data - Device payload data
 * @returns {string|null} - Raw mode string or null
 */
...
/**
 * Extract the battery level for legacy and M-series payloads.
 *
 * @param {object} data - Device payload data
 * @returns {number|null} - Battery level percentage
 */
...
/**
 * Extract the active error code for legacy and M-series payloads.
 *
 * @param {object} data - Device payload data
...
function errorCode(data) {
    return asInteger(
        /** @type {string|number|boolean|null|undefined} */ (firstPresent(data?.err_code, safeGet(data, 'error', 'value'))),
    );
}

/**
 * Extract the connected WiFi SSID for legacy and M-series payloads.
 *
 * @param {object} data - Device payload data
 * @returns {string|null} - WiFi SSID or null
 */
...
/**
 * Extract the device IP address for legacy and M-series payloads.
 *
 * @param {object} data - Device payload data
...
/**
 * Extract the SIM CCID for legacy and M-series payloads.
 *
 * @param {object} data - Device payload data
 * @returns {string|null} - SIM CCID or null
 */
...
/**
 * Determine whether a SIM is present.
 *
 * @param {object} data - Device payload data
 * @returns {boolean} - True when the payload indicates a SIM is present
 */
...
/**
 * Extract the mapped area for legacy and M-series payloads.
 *
 * @param {object} data - Device payload data
 * @returns {number|null} - Map area in square meters
 */
function mapArea(data) {
    return asNumber(/** @type {string|number|null|undefined} */ (firstPresent(data?.map_area, safeGet(data, 'map', 'map_area'))));
}

/**
 * Extract the M-series mapping task state.
 *
 * @param {object} data - Device payload data
 * @returns {string|null} - Mapping task state or null
 */
...
/**
 * Extract the total mowing time reported by M-series devices.
 *
 * @param {object} data - Device payload data
 * @returns {number|null} - Total mowing time
 */
...
/**
 * Extract the total mowing area reported by M-series devices.
 *
 * @param {object} data - Device payload data
 * @returns {number|null} - Total mowing area
 */
...
function rtkStateLabel(data) {
    ...
    const code = asInteger(/** @type {string|number|boolean|null|undefined} */ (firstPresent(rawState, data?.rtk_state)));
...
class AnthbotShadowApiClient {
     ...
     * @param {object|null} [config.accountClient] - Optional cloud client for forced credential refresh
     * @param {object|null} [config.iotCredentials] - Optional temporary IoT credentials
     * @param {string|null} [config.deviceModel] - Optional mower model string
     */
    constructor({
        http,
        serialNumber,
    ...
    /**
     * Store refreshed temporary IoT credentials on the client.
     *
     * @param {object|null} iotCredentials - Fresh credentials
     */
    ...
    /**
     * Force-refresh temporary IoT credentials using the configured cloud client.
     *
    ...
    /**
     * Determine whether a response status should trigger credential refresh.
     *
     * @param {number} status - HTTP status
     * @returns {boolean} - True for 403 responses
     */
     ...
     *
     * @param {string|number|boolean|null|undefined} value - Header value
     ...
     * @param {boolean} [allowCredentialRefresh] - Retry once after forced credential refresh on 403
    ...
    /**
     * Encode the command payload expected by M5/M9 models.
     *
     * @param {string} cmd - Command name
     * @param {unknown} data - Command data
     * @returns {unknown} - Encoded command data
     */
        ...
        if (cmd === 'volume_ctl') {
            if (data && typeof data === 'object' && !Array.isArray(data)) {
                ...
                const volume = asInteger(
                    /** @type {string|number|boolean|null|undefined} */ (firstPresent(encoded.volume_ctl, encoded.volume)),
                );
                if (volume != null) {
    ...
    /**
     * Build the shadow command payload for the current mower model.
     *
     * @param {object} params - Command parameters
     * @param {string} params.cmd - Command name
     * @param {unknown} [params.data] - Optional command data
     * @returns {object} - Shadow update body
     */
 ...
 *
 * @param {Array<*>|string|number|null|undefined} value - Command selection input
