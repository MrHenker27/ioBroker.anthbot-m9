 ...
 *
 * @param {unknown} deviceModel - Device model string
...
function batteryLevel(data) {
    return asInteger(firstPresent(safeGet(data, 'elec', 'value'), data?.elec));
}
...
function errorCode(data) {
    return asInteger(firstPresent(data?.err_code, safeGet(data, 'error', 'value')));
}
...
function mapArea(data) {
    return asNumber(firstPresent(data?.map_area, safeGet(data, 'map', 'map_area')));
}
...
function rtkStateLabel(data) {
    ...
    const code = asInteger(firstPresent(rawState, data?.rtk_state));
    ...
    static encodeMSeriesCommandData(cmd, data) {
        if (cmd === 'param_set') {
            if (data && typeof data === 'object' && !Array.isArray(data)) {
                const encoded = { ...data };
                const cutterLift = asInteger(firstPresent(encoded.cutter_ctl_cutter_lift, encoded.cutter_height));
                if (cutterLift != null) {
            ...
            const cutterLift = asInteger(data);
            return cutterLift == null ? data : { cutter_ctl_cutter_lift: cutterLift };
        }
        if (cmd === 'volume_ctl') {
            if (data && typeof data === 'object' && !Array.isArray(data)) {
                const encoded = { ...data };
                const volume = asInteger(firstPresent(encoded.volume_ctl, encoded.volume));
                if (volume != null) {
